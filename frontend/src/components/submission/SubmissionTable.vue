...
/**
 * Check if item can be edited (only by submitter)
 */
function canEdit(item: SubmissionListItem): boolean {
  return item.role !== 'coauthor' && (item.status === 'draft' || item.status === 'revision_required')
}

/**
 * Check if item can be deleted (only by submitter)
 */
function canDelete(item: SubmissionListItem): boolean {
  return item.role !== 'coauthor' && item.status === 'draft'
}
        ...
        <table class="w-full">
          ...
          <tbody class="divide-y divide-gray-50">
            ...
            >
              ...
              <td class="px-6 py-4">
                <span
                  v-if="item.role === 'coauthor'"
                  class="inline-flex items-center gap-1 px-2.5 py-1 bg-amber-50 text-amber-700 border border-amber-200 rounded-full text-xs font-medium"
                >
                ...
                <span
                  v-else
                  class="inline-flex items-center gap-1 px-2.5 py-1 bg-primary-50 text-primary-700 border border-primary-200 rounded-full text-xs font-medium"
                >
                  <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
